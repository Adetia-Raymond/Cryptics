...
export default function DashboardScreen() {
  ...
  const loadData = useCallback(async (opts: { refresh?: boolean } = {}) => {
    try {
      ...
      else if (Array.isArray((summariesResp as any).summaries))
        summaries = (summariesResp as any).summaries;
      ...
      setHighlight({
        technical: tech,
        summary:
          summaries.find((s: any) => s.symbol?.toUpperCase() === "BTCUSDT") ||
          summaries[0],
      });
      ...
      <View style={[styles.header, { borderBottomColor: border }]}>
        <View style={styles.headerRow}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <TouchableOpacity onPress={() => toggle()} style={[styles.themeToggle, { backgroundColor: cardBg, borderColor: border }]}>
              <MaterialCommunityIcons 
                name={effective === 'dark' ? 'white-balance-sunny' : 'moon-waning-crescent'} 
                size={20} 
                color={tint} 
              />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => router.push('/settings')} style={[styles.avatar, { backgroundColor: tint }]}> 
              <Text style={{ color: '#fff', fontWeight: '700' }}>{userInitials}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
      ...
      >
        ...
        <View style={{ marginTop: 16 }}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: text }]}>Markets</Text>
            <TouchableOpacity
              style={{
                backgroundColor: "rgba(138, 92, 246, 0.34)",
                paddingVertical: 5,
...
function HighlightCard({ data, text, muted, tint }: any) {
  ...
  const rsiColor = rsiLower.includes("overbought")
    ? "#FB923C"
    : rsiLower.includes("oversold")
    ? "#10B981"
    : muted;
  ...
  return (
    ...
    >
        ...
        >
          ...
          <View style={{ flexDirection: "column" }}>
            <Text style={{ color: muted, fontSize: 12 }}>
              {signal ? signal.toString() : "Technical"}
            </Text>
            <Text
              style={{
                color: text,
      ...
      >
        ...
        <Text style={{ color: muted, fontSize: 11, marginTop: 14 }}>
          {formattedTime}
        </Text>
      </View>
    </TouchableOpacity>
...
function MarketRow({ item, text, muted, cardBg, border }: any) {
  ...
  return (
    <View
      style={[
        styles.rowCard,
        { backgroundColor: rowBg, borderColor: border || "transparent" },
      ]}
    >
      <View
        style={{
